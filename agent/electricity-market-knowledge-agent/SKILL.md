...
网页展示还必须保持整体美学设计：头部使用通过 ChatGPT 图片生成能力制作的电力元素背景图，包含输电塔、电网线、数字化节点等电力市场意象；不得退回纯色背景。头部不展示英文标题，中文标题和元信息排版紧凑清晰，避免在 70px 高度内出现拥挤、错位或主次不清。页面整体采用专业、清爽、可信赖的知识库门户风格，头部固定、紧凑工具栏、搜索框和页签固定、表格区域滚动、知识摘要点击/悬浮浮层、Markdown 样式安全渲染、长文本折叠、弹窗只展示详细解读、导出 Excel 按钮、链接按钮、搜索框、页签和表格层次需要保持一致的阅读体验。
